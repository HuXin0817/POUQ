...
class Quantizer {
  ...
  void
  train_one_dim(const float* data_map,
                const int* freq_map,
                int data_size,
                const float* data,
                int size,
                ...
                uint8_t* cid,
                uint8_t* code,
                const optimize::Parameter& parameter) const {
    auto bounds = segment::segment<do_count_freq>(4, data_map, freq_map, data_size);

    for (int i = 0; i < bounds.size(); i++) {
      auto [lower, upper] = bounds[i];
      if (lower < upper) {
        auto data_begin = std::lower_bound(
            data_map, data_map + data_size, lower, [](float lhs, float rhs) -> bool {
              return lhs < rhs;
            });
        auto data_end = std::upper_bound(
            data_map, data_map + data_size, upper, [](float lhs, float rhs) -> bool {
              return lhs < rhs;
            });

        auto data_index = data_begin - data_map;

        std::tie(lower, upper) = optimize::optimize<do_count_freq>(
            3, lower, upper, data_begin, freq_map + data_index, data_end - data_begin, parameter);
      }
      lowers[d * 4 + i] = lower;
      if (lower == upper) {
        steps[d * 4 + i] = 1.0;
  ...
  int dim_ = 0;
  simd::Ptr<simd::RecPara> rec_para_;
  simd::Ptr<simd::CodeUnit> code_;
  ...
  bool
  train(const float* data, int size, const optimize::Parameter& parameter = optimize::Parameter()) {
    assert(code_.get() == nullptr);
    assert(rec_para_.get() == nullptr);
    assert(data != nullptr);
    assert(size > 0);
    assert(size % dim_ == 0);

    code_.init(size / 8 * sizeof(simd::CodeUnit));
    if (!code_.get()) {
      return false;
    }
    rec_para_.init(dim_ * 64 * sizeof(simd::RecPara));
    if (!rec_para_.get()) {
      return false;
    }
...
#define arg_list                                                                                 \
  data_map.data(), freq_map.data(), data_map.size(), data, size, d, steps.data(), lowers.data(), \
      cid.data(), code.data(), parameter

      if (do_count_freq) {
        std::tie(data_map, freq_map) = count_freq(data_map);
        train_one_dim<true>(arg_list);
      } else {
        train_one_dim<false>(arg_list);
      }
    }
...
#pragma omp parallel for
    for (int g = 0; g < dim_ / 4; g++) {
      for (int j = 0; j < 256; j++) {
        ...
        simd::set_rec_para(&rec_para_.get()[g * 256 + j],
                           lowers[x0],
                           lowers[x1],
                           lowers[x2],
                           lowers[x3],
  ...
  float
  distance(const float* data, int offset) const {
    return simd::distance(dim_, code_.get(), rec_para_.get(), data, offset);
  }
};
