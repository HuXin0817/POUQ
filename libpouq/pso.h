...
std::pair<float, float> optimize(const float                       div,
  ...
  for (uint64_t i = 0; i < grid_side_length; i++) {
    for (uint64_t j = 0; j < grid_side_length; j++) {
      const float lower_bound = initial_min_bound - grid_scale_factor * initial_range_width +
                                static_cast<float>(i) * 2 * grid_scale_factor * initial_range_width / grid_side_length;
      const float upper_bound = initial_max_bound - grid_scale_factor * initial_range_width +
