...
optimize(float div,
    ...
    float global_min_loss =
        loss(div, init_lower_bound, init_range_width / div, data_start, data_end);

    for (auto& particle : swarm) {
        const float curr_lower_bound = particle.center - particle.width * 0.5f;
        const float curr_loss =
            loss(div, curr_lower_bound, particle.width / div, data_start, data_end);
    ...
    for (size_t iter = 0; iter < max_iter; ++iter) {
        ...
        for (auto& particle : swarm) {
            ...
            const float curr_loss =
                loss(div, curr_lower, particle.width / div, data_start, data_end);
