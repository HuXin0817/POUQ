...
train_impl(int dim_,
           ...
           const float* data,
           int size,
           const optimize::Parameter& parameter = optimize::Parameter()) {
  assert(data != nullptr);
  assert(size > 0);
  assert(size % dim_ == 0);
...
#pragma omp parallel for
  for (int d = 0; d < dim_; d++) {
    ...
    auto bounds =
        segment::segment(4, data_map.data(), freq_map.data(), data_map.size(), do_count_freq);
    ...
    for (int i = 0; i < bounds.size(); i++) {
      ...
      if (lower < upper) {
        ...
        std::tie(lower, upper) = optimize::optimize(3,
                                                    lower,
                                                    upper,
                                                    data_begin,
