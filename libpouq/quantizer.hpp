...
#pragma omp parallel for
        for (size_t d = 0; d < dim_; d++) {
                ...
                if (lower < upper) {
                    const auto data_start =
                        std::lower_bound(data_freq_map.begin(),
                                         data_freq_map.end(),
                                         lower,
                                         [](const std::pair<float, size_t>& lhs,
                                            const float rhs) -> bool { return lhs.first < rhs; });
                    const auto data_end = std::upper_bound(
                        data_freq_map.begin(),
                        data_freq_map.end(),
                        upper,
                    ...
                    const auto [opt_lower, opt_upper] =
                        optimize(3, lower, upper, data_start, data_end);
                    lower = opt_lower;
                    upper = opt_upper;
                }
            ...
            for (size_t i = d; i < size; i += dim_) {
                const auto it =
                    std::upper_bound(bounds.begin(),
                                     bounds.end(),
                                     data[i],
                                     [](float rhs, const std::pair<float, float>& lhs) -> bool {
                                         return rhs < lhs.first;
                                     });
                const size_t c = it - bounds.begin() - 1;
                const float x = std::clamp(
                    (data[i] - lower_bound[d_times_4 + c]) / step_size[d_times_4 + c] + 0.5f,
                    0.0f,
                    3.0f);
